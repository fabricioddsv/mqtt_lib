...
typedef struct {
    ...
    const char* username;
    const char* password;
    mqtt_message_callback_t on_message_callback; // Sua função para lidar com mensagens recebidas
} mqtt_config_t;

/**
 * @brief Inicializa o Wi-Fi e conecta ao broker MQTT de forma não-bloqueante.
 * A função principal da aplicação deve continuar chamando cyw43_arch_poll() em seu loop.
